...
uint8_t* Converter::RGB2YCbCr(uint8_t* rgb) {
    int     YCbCr[3];
...
uint8_t* Converter::YCbCr2RGB(uint8_t* ycbcr) {
    ...
    YCbCr[0] = ycbcr[0];
    YCbCr[1] = ycbcr[1] - 128.0;
    YCbCr[2] = ycbcr[2] - 128.0;
...
uint8_t* Converter::RGB2HSV(uint8_t* rgb) {
    double MAX, MIN;
    double HSV[3];
    double RGB[3];
    RGB[0] = rgb[0] / 255.0; // red
    RGB[1] = rgb[1] / 255.0; // green
    RGB[2] = rgb[2] / 255.0; // blue
    MAX = max(RGB[0], max(RGB[1], RGB[2]));
    MAX = min(RGB[0], min(RGB[1], RGB[2]));

    if(MAX == MIN)                                  HSV[0] = 0;
    else if(MAX == RGB[0] && RGB[1] >= RGB[2])      HSV[0] = 60 * (RGB[1] - RGB[2]) / (MAX - MIN);
    else if(MAX == RGB[0] && RGB[1] <  RGB[2])      HSV[0] = 60 * (RGB[1] - RGB[2]) / (MAX - MIN) + 360.0;
    else if(MAX == RGB[1])                          HSV[0] = 60 * (RGB[2] - RGB[0]) / (MAX - MIN) + 120.0;
    else if(MAX == RGB[2])                          HSV[0] = 60 * (RGB[0] - RGB[1]) / (MAX - MIN) + 240.0;

    HSV[1] = MAX == 0 ? 0 : 1.0 - MIN / MAX;
    HSV[2] = MAX;


    HSV[0] = (HSV[0] / 360.0) * 255.0;
    HSV[1] = HSV[1] * 255.0;
    HSV[2] = HSV[2] * 255.0;
...
uint8_t* Converter::HSV2RGB(uint8_t* hsv) {
    double MAX, MIN;
    double  RGB[3];
    double  HSV[3];

    HSV[0] = (hsv[0] / 255.0) * 360.0; // hue
    HSV[1] = hsv[1] / 255.0;           // saturation
    HSV[2] = hsv[2] / 255.0;           // value

    double Hi = ((int) (HSV[0] / 60.0)) % 6;
